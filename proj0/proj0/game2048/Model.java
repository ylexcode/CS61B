...
public class Model extends Observable {
    ...
    public boolean tilt(Side side) {
        ...
        for (int col = 0; col < size; col++) {
            ...
            for (int row = size - 2; row >= 0; row--) { // 从倒数第二行开始向上遍历
                ...
                // 合并相邻瓷砖
                if (targetRow + 1 < size && !merged[targetRow + 1] && board.tile(col, targetRow + 1).value() == t.value()) {
                    board.move(col, targetRow + 1, t);
                    score += 2 * t.value();
                    merged[targetRow + 1] = true;
                    changed = true;
                } else if (targetRow != row) { // 移动瓷砖到空位
    ...
    public static boolean atLeastOneMoveExists(Board b) {
        ...
        // Check if there's at least one empty space on the board
        ...
        for (int col = 0; col < size; col++) {
            for (int row = 0; row < size; row++) {
                Tile t = b.tile(col, row); // 获取位于 (col, row) 的瓷砖对象
                if (t == null) continue; // 如果瓷砖为空，跳过
